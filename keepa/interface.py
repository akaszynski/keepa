        ...
        if stat_value is not None:
            if stat_key == 'lastOffersUpdate':
                ...
                for ind, key, isfloat in csv_indices:
                    stat_value_item = stat_value[ind] if ind < len(
                        stat_value) else None
                        ...
                        if convert_time_in_value_pair:
                            ...
                            if stat_value_item is not None:
                                stat_value_time = keepa_minutes_to_time(
                                    [stat_value_time], to_datetime)[0]
                                stat_value_item = (
                                    stat_value_time, stat_value_item)
                        else:
                            stat_value_item = normalize_value(stat_value_item)
...
def _parse_seller(seller_raw_response, to_datetime):
    ...
    for seller in sellers:
        ...
        seller.update(filter(lambda p: p is not None, map(
            convert_time_data, _seller_time_data_keys)))
...
def parse_csv(csv, to_datetime=True, out_of_stock_as_nan=True):
    ...
    for ind, key, isfloat in csv_indices:
        if csv[ind]:  # Check if entry it exists
            ...
            product_data[f'df_{key}'] = pd.DataFrame(
                {'value': values}, index=timeval)
...
class Keepa():
    ...
    def time_to_refill(self):
        ...
        if self.tokens_left < 0:
            timetorefil += (abs(self.tokens_left) /
                            self.status['refillRate']) * 60000
    ...
    def update_status(self):
        ...
        self.status = self._request(
            'token', {'key': self.accesskey}, wait=False)
    ...
    def query(self, items, stats=None, domain='US', history=True,
        ...
        if offers:
            ...
            if offers > 100 or offers < 20:
                raise ValueError(
                    'Parameter "offers" must be between 20 and 100')
...
class AsyncKeepa():
    ...
    async def query(self, items, stats=None, domain='US', history=True,
        ...
        if raw:
            raise ValueError(
                'Raw response is only available in the non-async class')
